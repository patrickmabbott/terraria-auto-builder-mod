        ...
        public static void PlaceFurniture(int x, int y, Vector2 position, int side, Player player)
        {
            ...
            if (y == -2)
            {
                ...
                if (x == (5 * side))
                {
                    ...
                    int tileId = TileID.Chairs;

                    foreach(Item item in player.inventory)
                    {
                        if(item.createTile == TileID.Chairs || item.Name.Contains("Chair"))
                        {
                    ...
                    WorldGen.PlaceObject(xPosition, yPosition, tileId, direction: side, style: placeStyle);
                    if (Main.netMode == NetmodeID.Server)
                        NetMessage.SendData(MessageID.TileChange, -1, -1, null, 1, xPosition, yPosition, tileId, placeStyle);
                }
